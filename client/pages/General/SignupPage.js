...
const SignupPage = () => {
  ...
  //   // Capture 100 images with face detection
  //   for (let i = 0; i < 100; i++) {
  ...
  //       // Face detected, upload the image to the database
  //       await uploadImageToDatabase(uri);
  //     } else {
  ...
  //   Alert.alert("Success", "Image capture completed");
  // };

  const uploadImageToDatabase = async (imageUri) => {
    try {
      const response = await fetch(imageUri);
      const blob = await response.blob();
      const imageRef = ref(
        storage,
        `user-images/${signUpForm.userType}/${Date.now()}-${Math.random()
          .toString(36)
          .substr(2, 9)}`
      );
...
const styles = StyleSheet.create({
  ...
  captureButton: {
    backgroundColor: "#4a90e2", // Blue capture button
    borderRadius: 6,
    padding: 15,
    alignItems: "center",
    justifyContent: "center",
