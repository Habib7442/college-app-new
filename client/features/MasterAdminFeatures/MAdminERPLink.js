...
const MAdminERPLink = () => {
  ...
  const fetchERPLinks = async () => {
    ...
    try {
      ...
      Alert.alert("Error", "Failed to fetch App/Web links");
      console.error("Error fetching App/Web links: ", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddERPLink = async () => {
    if (erpName.trim() !== "" && erpURL.trim() !== "" && isValidURL(erpURL)) {
      ...
      try {
        ...
        setERPURL("");
        setError("");
        Alert.alert("Success", "App/Web link added successfully");
      } catch (error) {
        setError("Failed to App/Web link.");
        console.error("Error adding App/Web link: ", error);
      } finally {
        setLoading(false);
      }
    } else {
  ...
  const handleDeleteERPLink = async (index) => {
    ...
    try {
      ...
      Alert.alert("Success", "App/Web link deleted successfully");
    } catch (error) {
      setError("Failed to delete App/Web link.");
      console.error("Error deleting App/Web link: ", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safeContainer}>
      <ScrollView contentContainerStyle={styles.container}>
        <Text style={styles.heading}>App/Web Links</Text>
        <TextInput
          style={styles.input}
          placeholder="Enter App/Web Name"
          onChangeText={(value) => setERPName(value)}
          value={erpName}
        />
        <TextInput
          style={styles.input}
          placeholder="Enter App/Web Link"
          onChangeText={(value) => setERPURL(value)}
          value={erpURL}
        />
        {error ? <Text style={styles.errorText}>{error}</Text> : null}
        <TouchableOpacity onPress={handleAddERPLink} style={styles.button}>
          <Text style={styles.buttonText}>Add App/Web Link</Text>
        </TouchableOpacity>
        {loading ? (
